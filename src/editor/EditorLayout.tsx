...
export default function EditorLayout() {
  ...
  return (
    <div className="h-screen w-screen bg-surface-900 text-slate-200 flex flex-col overflow-hidden">
      {/* Top Menu Bar */}
      <div className="h-10 bg-surface-800 border-b border-surface-600 flex items-center px-3 gap-2 shrink-0">
        <div className="flex items-center gap-2 mr-4">
          ...
          <input
            ...
            className="bg-transparent text-sm font-medium text-slate-300 hover:text-white focus:text-white outline-none border-b border-transparent focus:border-accent px-1 min-w-[120px]"
          />
        </div>

        <div className="w-px h-5 bg-surface-600" />

        <button onClick={() => showToast('Project saved')} className="flex items-center gap-1.5 px-2 py-1 rounded hover:bg-surface-700 text-xs text-slate-300">
          <Save size={13} />
          Save
        </button>
        ...
        <div className="px-3 py-1 rounded bg-surface-700 border border-surface-600">
